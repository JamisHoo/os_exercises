...
```
 ...
 ```
- [x]  
	- 对中断描述符表IDT进行初始化。为中断号和中断服务例程建立索引。
	- 对中断控制器进行初始化。
	- 对时钟进行初始化。
	- 对中断进行使能。
...
- [x]  
	-1、 首先实现一个可显示字符串的bootloader。其中关键步骤是对时钟、串口和CGA的正常访问，以及从实模式切换到保护模式。使得段机制可以正常运行。
	-2、 在实现一个可加载ELF格式文件的bootloader。关键步骤是对串、并口的正常访问。从而可以加载磁盘扇区。将ucore代码从硬盘中读取到内存指定位置，并将控制权交给ucore。
> 
...
- [x]  
	- 使用strace指令。
	- 修改操作系统代码，使得程序可以在进入中断向量表和系统调用表之间时，跳转到一段代码中记录当前系统调用类型。之后再返回进行系统调用表的访问操作。
