...
>  
- 对GDT完成了初始化，完成了段机制
- 对物理内存进行探测，找到空闲的物理内存，进行线性地址的映射转换
- 建立页表
...
- [x]  
- 退出是因为没有初始化中断处理向量idt，所以当操作系统执行出现中断或异常的时候（例如时钟中断，串口或者键盘的中断），操作系统不知道应该如何进行响应。因此就会出现错误导致退出。
...
- [x]  
- 我的学号是2012011300, 模37结果是29。
- 加上该语句后程序不能正常运行，且在屏幕上显示如下：

kernel panic at kern/trap/trap.c:182:
    unexpected trap in kernel.
- 经过查找kern/trap/trap.h可以发现是T_STACK，应该是表示某一种栈错误。而在trap_dispatch函数中并未对这种情况进行处理，因此导致程序无法运行。
...
1100 1101 1000 0010 1100 0000 0111 1100          0000 1100 0010 0000 1001 0000 0111 1100
0000 0000 0000 0000 0000 0011 0011 0110          0000 0000 0000 0011 0111 0000 0000 0011
0000 0000 0000 0000 0000 0000 0010 1100          0000 0000 0000 0000 1100 0010 0000 1011
```
...
``` 
 ...
 void decode(unsigned int num, unsigned int* p1, unsigned int* p2, unsigned int* p3){ 
   *p3 = num % 4096;//低12位 
   num = num / 4096; 
   *p2 = num % 1024;//中间10位 
   num = num / 1024; 
 ...
 int main(){
   ...
   for(int i=0; i<11; i++){
     ...
     pde_idx = p1, pte_idx = p2;
     pte_ctx = (pa & 0xfffff000) | 0x3;
     unsigned int high10 = (p1 & 0x000000ff) + 0x1;
     pde_ctx = ((high10 << 12) & 0xfffff000) | 0x3; 
