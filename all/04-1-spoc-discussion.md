...
- 将原有程序中的A[i][j]替换成A[j][i]，然后运行更改后的代码，发现运行时间明显变长，分析原因为原来的先行后列的赋值形式变为先列后行形式导致了内存的访问局部性被破坏，所以程序的执行时间会比原来长。
...
```
具体计算方法是先将虚拟地址展开成2进制串，如0x6653展开成0110011001010011,之后将该串分成三部分（0 11001 10010 10011）。由于PDBR是0xd80(110110000000)，取其前7位，得到页号page 6c。之后根据11001获得偏移地址0x19。则在page 6c中找到对应的内容为0x7f(01111111)。发现该内容的标志位为0,表示该内容不存在，因此出现缺失状况。
若虚拟地址为0x1c13（0 00111 00000 10011）时，在page 6c中寻找0x7的内容，发现为0xbd(10111101)，其中标志位（最高位）为1,则存在该页表。根据其后7位获得0x3d。之后去page 3d中寻找0x0（根据虚拟地址中间5位），其内容为0xf6（11110110），标志位为1,由此获得其内容0x76。将0x76作为高7位，与10011合并获得物理地址0xed3。由于0xf6标志位为1,因此在内存中page 76寻找 索引0x13对应的内容为0x12。因此获得所需值。
```
